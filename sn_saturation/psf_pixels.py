...
class PixelPSFSeeing:
    def __init__(self, psf_type, scanfast=False):
        """
        class to estimate the flux fraction max 
        as a function of the seeing depending on PSF

        Parameters
        ----------
        psf_type : str
            psf to use.
        scanfast : bool, optional
            To speed up the process (price: lower resolution). 
    ...
    def summary(self, resfi):
        ...
        Returns
        -------
        finalresu : record array
            Final results with the following columns:
    ...
    def loop_seeing(self):
        ...
        for seeing in self.seeings:
            toproc = PSF_pixels(seeing, self.psf_type, scanfast=self.scanfast)
